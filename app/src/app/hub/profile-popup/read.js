...
/**
 * The one read behind every profile surface in the app — the popup and the
 * full profile page both.
 *
 ...
 *
 * The projection is what enforces that, rather than a rule in a component.
 ...
 * fields the popup must not show never leave Postgres, and no later edit in a
 * component can leak them.
 *
 * A plain module rather than the `'use server'` file next door, because a
 * Server Component reads this too. Importing a server action into a page works
 * but registers a needless client-callable endpoint for a read the server was
 * going to do itself; `actions.js` re-exports it for the popup, which is a
 * Client Component and genuinely needs the action.
 */
